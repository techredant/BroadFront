...
export default function CommentModal({
  ...
  React.useEffect(() => {
    const postId = String(postCard?._id ?? "");
    if (!socket || !visible || !postId) return;

    const onCommentAdded = (payload: { postId?: string; comment?: any }) => {
      if (String(payload?.postId ?? "") !== postId || !payload?.comment) return;
      const incoming = payload.comment;
      const incomingId = String(incoming._id ?? "");

      setComments((prev: any[]) => {
        if (prev.some((c) => String(c._id) === incomingId)) return prev;
        const withoutMatchingTemp = prev.filter((c) => {
          if (!String(c._id ?? "").startsWith("temp-")) return true;
          return !(c.userId === incoming.userId && c.text === incoming.text);
        });
        return [...withoutMatchingTemp, incoming];
      });
    };

    const onCommentUpdated = (payload: { postId?: string; comment?: any }) => {
      if (String(payload?.postId ?? "") !== postId || !payload?.comment) return;
      const incoming = payload.comment;
      const incomingId = String(incoming._id ?? "");

      setComments((prev: any[]) =>
        prev.map((c) => (String(c._id) === incomingId ? incoming : c)),
      );
    };

    const onCommentDeleted = (payload: { postId?: string; commentId?: string }) => {
      if (String(payload?.postId ?? "") !== postId || !payload?.commentId) return;
      setComments((prev: any[]) =>
        prev.filter((c) => String(c._id) !== String(payload.commentId)),
      );
    };
  ...
  }, [postCard?._id, setComments, socket, visible]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${API_PUBLIC_URL}/api/comments/${postCard?._id}/comments`,
        {
          method: "POST",
  ...
  const handleSubmitReply = async () => {
    ...
    try {
      const res = await axios.post(
        `${API_PUBLIC_URL}/api/comments/${commentId}/replies`,
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    try {
      await axios.delete(
        `${API_PUBLIC_URL}/api/comments/${commentId}`,
        { data: { userId } },
      );
    } catch {
      setComments(prev);
  ...
  const handleDeleteReply = async (commentId: string, replyId: string) => {
    ...
    try {
      await axios.delete(
        `${API_PUBLIC_URL}/api/comments/${commentId}/replies/${replyId}`,
      );
    } catch {
      setComments(prev);
    }
  ...
  const handleLikeReply = async (commentId: string, replyId: string) => {
    ...
    try {
      await axios.post(
        `${API_PUBLIC_URL}/api/comments/${commentId}/replies/${replyId}/like`,
        { userId },
      );
    } catch (err) {
      console.log(err);
  ...
  const handleLike = async (commentId: string) => {
    ...
    try {
      await axios.post(
        `${API_PUBLIC_URL}/api/comments/${commentId}/like`,
        { userId },
      );
    } catch (err) {
      console.log(err);
